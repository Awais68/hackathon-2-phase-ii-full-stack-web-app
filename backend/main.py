...
async def list_tasks(
    ...
    for task in tasks:
        result.append({
            ...
            "tags": task.tags if isinstance(task.tags, list) else (task.tags.split(",") if task.tags else [])
        })
...
async def create_task(todo: TodoCreate, db: Session = Depends(get_db)):
    ...
    # Create task in database (id is auto-generated by database)
    db_task = TaskDB(
        title=todo.title,
    ...
    return {
        ...
        "tags": db_task.tags if isinstance(db_task.tags, list) else (db_task.tags.split(",") if db_task.tags else [])
    }
